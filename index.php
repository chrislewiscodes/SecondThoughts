...
require_once("dev/parsedown.php");
...
function section($section) {
	...
	foreach ($files as $i => $filename) {
		...
		$revisions[$i][$section] = file_get_contents($fullpath);
		if ($i > 0) {
			$diffs[$i][$section] = htmlDiff($revisions[$i-1][$section], $revisions[$i][$section]);
		} 

		if ($i===0) {
			print markdown($revisions[$i][$section]);
		} else {
			//print markdown($diffs[$i][$section]);
		}
	}
}
...
<div id="counter">
	Last Updated: July, 1<span class="number">3</span>, 2015
</div>
<!-- current version -->
<div class="version">
	<div class="content">
...
<div id="overlay" class="overlay-on">
	<div id="message">
		...
		<p>
			...
			Last Updated: <span id='overlay_date'><?php echo date('F j, Y', $revisiondates[0]); ?></span>
		</p>
	</div>
</div>
