...
/* MODIFIED HTMLOutput::parse to output our desired tags 
 * by Chris Lewis <chris@chrislewis.codes>, August 2015	
*/
		...
		foreach ($node->children as &$child) {
			...
			if ($child instanceof TagNode) {
				...
				if (!$newStarted && $mod->type == Modification::ADDED) {
					...
					//tooltip
					$handler->startElement('aside', array('class' => 'tip'));
					$handler->html($mod->changes);
					$handler->endElement('aside');
